...
function Home() {
    ...
    const {profilelink}=location.state
    ...
    useEffect(() => {
        const fetchUser = async () => {
            const res = await axios.get("http://localhost:8000/users/profile/"+JSON.parse(profilelink));
            console.log("http://localhost:8000/users/profile/"+JSON.parse(profilelink));
           console.log(res);
            setUser(res.data[0]);
        };
    ...
    return (
        <>
                    ...
                    <div className="profile-page-introduction-left">
                        ...
                        <div className="workStatus">
                            <div className="workStatus">
                                ...
                                <form action="/Contact">
                                    <button className="profile-buttons">{work ? "Open to work" : "Already on Work"}</button>
                                </form>
                            </div>
                        </div>
                    </div>
            ...
            <Highlights 
            heading="Education" 
            // para=" Aliquam optio reprehenderit nostrum asperiores deserunt iure dicta alias repudiandae, nulla porro? Sunt minus quod nihil ipsam id eligendi, suscipit, dolores tempora modi incidunt quas? Harum ratione cum a maxime!
            //             El"
            para={user.education}/>
            <Highlights heading="Skills" 
            para={user.skills}/>
            <Highlights heading="Interests" 
             para={user.interests}
            />
        </>
    )
